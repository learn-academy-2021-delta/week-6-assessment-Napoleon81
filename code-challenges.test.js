...
describe("peopleInfo",() => {
  let people = [{ name: "ford prefect", occupation: "a hitchhiker"}]
  it("return an array with a sentence about each person with their name capitalized",() => {
expect(peopleInfo(people)).toEqual(result)
})
})
...
expect("dividedBy3"(hodgepodge1)).toEqual(result)
})
})
...
const numBy3 = (array) =>
 {
  return hodgepodge1.filter value => typeof value === "number").map(value => value % 3)
}
...
expect(numCubed(cubeAndSum1)).toEqual(result)
})
})
